"""Train quadruped locomotion directly in MuJoCo using PPO.
Eliminates sim-to-sim gap by training in the same physics engine used for deployment.
Supports loading pretrained Isaac Gym weights for fine-tuning."""
...
PRETRAINED = '/root/gpufree-data/workspace/legged_gym/logs/rough_dog_urdf/best_models/v8_iter650_best.pt'
SAVE_DIR = '/root/gpufree-data/workspace/legged_gym/logs/mujoco_train'
...
REWARD_SCALES = {
    ...
    'dof_acc': -2.5e-7, 'action_rate': -0.02,
    'feet_air_time': 5.0, 'foot_slip': -0.1,
}
...
CMD_RANGES = {'vx': [-1,1], 'vy': [-0.5,0.5], 'vyaw': [-1,1], 'heading': [-3.14,3.14]}
...
class MuJoCoVecEnv:
    def __init__(self, num_envs):
        ...
        self.commands = np.zeros((num_envs, 4), dtype=np.float32)
    ...
    def step(self, actions_np):
        ...
        target = actions_np * ACTION_SCALE + DEFAULT_ANGLES

        for i in range(self.n):
            d = self.datas[i]
            for _ in range(DECIMATION):
                tau = KP * (target[i] - d.qpos[7:]) + KD * (0 - d.qvel[6:])
                d.ctrl[:] = tau
                mujoco.mj_step(self.model, d)
            self.torques[i] = KP * (target[i] - d.qpos[7:]) + KD * (0 - d.qvel[6:])
    ...
    def _compute_obs(self, env_ids):
        for i in env_ids:
            ...
            heading_err = self.commands[i,3] - np.arctan2(2*(self.datas[i].qpos[3]*self.datas[i].qpos[6]+self.datas[i].qpos[4]*self.datas[i].qpos[5]), 1-2*(self.datas[i].qpos[5]**2+self.datas[i].qpos[6]**2))
            heading_err = (heading_err + np.pi) % (2*np.pi) - np.pi
            cmd[2] = np.clip(heading_err, -1, 1)
            self.commands[i,2] = cmd[2]

            self.obs[i, :3] = lv * OBS_SCALES['lin_vel']
            self.obs[i, 3:6] = av * OBS_SCALES['ang_vel']
            self.obs[i, 6:9] = g
            self.obs[i, 9:12] = cmd * CMD_SCALE
            self.obs[i, 12:24] = (qj - DEFAULT_ANGLES) * OBS_SCALES['dof_pos']
            self.obs[i, 24:36] = dqj * OBS_SCALES['dof_vel']
            self.obs[i, 36:48] = self.actions[i]
    ...
    def _compute_rewards(self):
        ...
        for i in range(self.n):
            ...
            r = {}
            lin_err = np.sum((cmd[:2] - lv[:2])**2)
            r['tracking_lin_vel'] = np.exp(-lin_err / TRACKING_SIGMA)
            ang_err = (cmd[2] - av[2])**2
            r['tracking_ang_vel'] = np.exp(-ang_err / TRACKING_SIGMA)
            r['lin_vel_z'] = lv[2]**2
            r['ang_vel_xy'] = np.sum(av[:2]**2)
            r['orientation'] = np.sum(g[:2]**2)
            r['base_height'] = (h - BASE_HEIGHT_TARGET)**2
...
def train(num_iters=500, num_steps=24, save_interval=50):
    ...
    print(f"MuJoCo VecEnv: {NUM_ENVS} envs, dt={SIM_DT}, dec={DECIMATION}")
...
if __name__ == '__main__':
    train(num_iters=500, num_steps=24, save_interval=50)
