"""MuJoCo PPO training with 46-dim frame stacking + privileged critic.
Supports action latency DR for sim-to-real transfer."""
...
REWARD_SCALES = {
    ...
    'orientation': -5.0, 'base_height': -1.0,
...
OBS_SCALES = {'ang_vel': 0.25, 'dof_pos': 1.0, 'dof_vel': 0.05, 'lin_vel': 2.0}
...
DR = {
    ...
    'obs_noise': {
        'ang_vel': 0.0, 'dof_pos': 0.0, 'dof_vel': 0.0, 'gravity': 0.0,
    },
}
...
class MuJoCoVecEnv:
    def __init__(self, num_envs):
        ...
        self.foot_body_ids = [mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, n) for n in ['FL_foot','FR_foot','RL_foot','RR_foot']]
        self.thigh_body_ids = [mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, n) for n in ['FL_thigh','FR_thigh','RL_thigh','RR_thigh']]
        self.calf_body_ids = [mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, n) for n in ['FL_calf','FR_calf','RL_calf','RR_calf']]
        ...
        self.actor_obs = np.zeros((num_envs, NUM_OBS), dtype=np.float32)
        self.critic_obs = np.zeros((num_envs, NUM_CRITIC_OBS), dtype=np.float32)
        self.obs_history = np.zeros((num_envs, NUM_OBS), dtype=np.float32)
    ...
    def reset(self, env_ids):
        for i in env_ids:
            ...
            d.qpos[2] = 0.25; d.qpos[3:7] = [1,0,0,0]
            d.qpos[7:] = DEFAULT_ANGLES * np.random.uniform(0.8, 1.2, 12)
            d.qvel[:] = 0; mujoco.mj_forward(self.model, d)
            self.actions[i] = 0; self.last_actions[i] = 0; self.last_dof_vel[i] = 0
            self.ep_steps[i] = 0; self.feet_air_time[i] = 0; self.last_contacts[i] = True
            self.obs_history[i] = 0
            self.action_buffer[i] = 0; self.act_buf_idx[i] = 0
    ...
    def step(self, actions_np):
        ...
        for i in range(self.n):
            ...
            d = self.datas[i]; kp, kd = self.env_kp[i], self.env_kd[i]
            for _ in range(DECIMATION):
                tau = kp * (target - d.qpos[7:]) + kd * (0 - d.qvel[6:])
                d.ctrl[:] = tau; mujoco.mj_step(self.model, d)
        ...
        return self.actor_obs.copy(), self.critic_obs.copy(), rewards, dones, {'time_outs': timeouts}
    ...
    def _compute_obs(self, env_ids):
        for i in env_ids:
            ...
            single[:3] = av * OBS_SCALES['ang_vel']
            single[3:6] = g
            single[6:9] = cmd * CMD_SCALE
            single[9] = self.commands[i, 4]
            single[10:22] = (qj - DEFAULT_ANGLES) * OBS_SCALES['dof_pos']
            single[22:34] = dqj * OBS_SCALES['dof_vel']
            single[34:46] = self.actions[i]

            n = NUM_SINGLE_OBS
            self.obs_history[i, :n*(FRAME_STACK-1)] = self.obs_history[i, n:]
            self.obs_history[i, n*(FRAME_STACK-1):] = single
            self.actor_obs[i] = self.obs_history[i]

            foot_contacts = np.array([d.xpos[fid][2] < 0.025 for fid in self.foot_body_ids], dtype=np.float32)
            self.critic_obs[i, :NUM_SINGLE_OBS] = single
            self.critic_obs[i, NUM_SINGLE_OBS:NUM_SINGLE_OBS+3] = lv * OBS_SCALES['lin_vel']
            self.critic_obs[i, NUM_SINGLE_OBS+3:NUM_SINGLE_OBS+7] = foot_contacts

    def _compute_rewards(self):
        rewards = np.zeros(self.n, dtype=np.float32)
        for i in range(self.n):
            lv, av, g, qj, dqj, h = self._get_state(i)
            cmd = self.commands[i, :3]; r = {}
            r['tracking_lin_vel'] = np.exp(-np.sum((cmd[:2] - lv[:2])**2) / TRACKING_SIGMA)
            r['tracking_ang_vel'] = np.exp(-(cmd[2] - av[2])**2 / TRACKING_SIGMA)
            r['lin_vel_z'] = lv[2]**2; r['ang_vel_xy'] = np.sum(av[:2]**2)
            r['orientation'] = np.sum(g[:2]**2)
            r['base_height'] = (h - self.commands[i, 4])**2
            r['torques'] = np.sum(self.torques[i]**2)
            r['dof_acc'] = np.sum(((dqj - self.last_dof_vel[i]) / POLICY_DT)**2)
            ...
            contact = foot_z < 0.025; contact_filt = contact | self.last_contacts[i]
            ...
            self.feet_air_time[i] *= ~contact_filt; self.last_contacts[i] = contact
    ...
    def _check_termination(self):
        ...
        for i in range(self.n):
            d = self.datas[i]
            if d.qpos[2] < 0.08: dones[i] = True
            _, _, g, _, _, _ = self._get_state(i)
            if abs(g[2]) < 0.3: dones[i] = True
        return dones

    def get_actor_obs(self):
        return torch.from_numpy(self.actor_obs)

    def get_critic_obs(self):
        return torch.from_numpy(self.critic_obs)


def train(num_iters=500, num_steps=24, save_interval=100):
    os.makedirs(SAVE_DIR, exist_ok=True)
    device = 'cpu'

    model = ActorCritic(NUM_OBS, NUM_CRITIC_OBS, NUM_ACTIONS, [512,256,128], [512,256,128], 'elu', 1.0)
    ...
    gamma, lam = 0.99, 0.95
    clip_param, entropy_coef, value_coef = 0.2, 0.01, 1.0
    num_epochs, num_minibatches = 5, 4
    desired_kl = 0.01; lr = 1e-5

    env = MuJoCoVecEnv(NUM_ENVS)
    print(f"MuJoCo VecEnv: {NUM_ENVS} envs, actor={NUM_OBS}d, critic={NUM_CRITIC_OBS}d, frame_stack={FRAME_STACK}")

    for iteration in range(num_iters):
        t0 = time.time()
        a_obs_buf = torch.zeros(num_steps, NUM_ENVS, NUM_OBS)
        c_obs_buf = torch.zeros(num_steps, NUM_ENVS, NUM_CRITIC_OBS)
        ...
        a_obs_t = env.get_actor_obs().float()
        c_obs_t = env.get_critic_obs().float()

        for step in range(num_steps):
            with torch.no_grad():
                mean = 4.0 * torch.tanh(model.actor(a_obs_t))
                std = model.std.expand_as(mean)
                dist = Normal(mean, std)
                actions = dist.sample()
                logp = dist.log_prob(actions).sum(-1)
                value = model.critic(c_obs_t).squeeze(-1)

            a_obs_buf[step] = a_obs_t; c_obs_buf[step] = c_obs_t
            act_buf[step] = actions; val_buf[step] = value
            logp_buf[step] = logp; mu_buf[step] = mean; sigma_buf[step] = std

            a_obs_np, c_obs_np, rewards, dones, infos = env.step(actions.numpy())
            rew_buf[step] = torch.from_numpy(rewards)
            done_buf[step] = torch.from_numpy(dones.astype(np.float32))
            a_obs_t = torch.from_numpy(a_obs_np).float()
            c_obs_t = torch.from_numpy(c_obs_np).float()

        with torch.no_grad():
            last_val = model.critic(c_obs_t).squeeze(-1)

        advantages = torch.zeros(num_steps, NUM_ENVS); last_gae = 0
        ...
        a_flat = a_obs_buf.reshape(-1, NUM_OBS)
        c_flat = c_obs_buf.reshape(-1, NUM_CRITIC_OBS)
        act_flat = act_buf.reshape(-1, NUM_ACTIONS)
        logp_flat = logp_buf.reshape(-1); ret_flat = returns.reshape(-1)
        adv_flat = advantages.reshape(-1)
        adv_flat = (adv_flat - adv_flat.mean()) / (adv_flat.std() + 1e-8)
        mu_flat = mu_buf.reshape(-1, NUM_ACTIONS)
        sigma_flat = sigma_buf.reshape(-1, NUM_ACTIONS)

        total = a_flat.shape[0]; batch_size = total // num_minibatches
        for epoch in range(num_epochs):
            perm = torch.randperm(total)
            for mb in range(num_minibatches):
                idx = perm[mb*batch_size:(mb+1)*batch_size]
                mean_new = 4.0 * torch.tanh(model.actor(a_flat[idx]))
                std_new = model.std.expand_as(mean_new)
                dist_new = Normal(mean_new, std_new)
                logp_new = dist_new.log_prob(act_flat[idx]).sum(-1)
                entropy = dist_new.entropy().sum(-1)
                val_new = model.critic(c_flat[idx]).squeeze(-1)

                ratio = torch.exp(logp_new - logp_flat[idx])
                surr1 = ratio * adv_flat[idx]
                surr2 = torch.clamp(ratio, 1-clip_param, 1+clip_param) * adv_flat[idx]
                loss = -torch.min(surr1, surr2).mean() + value_coef * ((val_new - ret_flat[idx])**2).mean() - entropy_coef * entropy.mean()

                optimizer.zero_grad(); loss.backward()
                nn.utils.clip_grad_norm_(model.parameters(), 1.0); optimizer.step()

            with torch.no_grad():
                mean_new2 = 4.0 * torch.tanh(model.actor(a_flat))
                ...
                if kl > desired_kl * 2: lr = max(1e-6, lr / 1.5)
                elif kl < desired_kl / 2 and kl > 0: lr = min(1e-3, lr * 1.5)
                for pg in optimizer.param_groups: pg['lr'] = lr

        elapsed = time.time() - t0
        mean_rew = rew_buf.sum(0).mean().item()
        print(f"Iter {iteration:4d}/{num_iters} | reward={mean_rew:6.2f} | std={model.std.mean().item():.3f} | lr={lr:.1e} | {elapsed:.2f}s")
    ...
    torch.jit.save(torch.jit.trace(wrapped, torch.zeros(1, NUM_OBS)), policy_path)
